...
prepare_driver_set(struct panvk_cmd_buffer *cmdbuf)
{
   ...
   struct panvk_shader_desc_state *shader_desc =
   ...
   /* Valhall texture fetches without an application sampler use table 0,
    * descriptor 0. Dynamic buffers follow it, as on the CSF compute path. */
   pan_cast_and_pack(&descs[0], SAMPLER, cfg) {
      cfg.clamp_integer_array_indices = false;
   }
   panvk_per_arch(cmd_fill_dyn_bufs)(
      &cmdbuf->state.compute.desc_state, desc_info,
      (struct mali_buffer_packed *)&descs[1]);
   shader_desc->driver_set.dev_addr = driver_set.gpu;
   shader_desc->driver_set.size = desc_count * PANVK_DESCRIPTOR_SIZE;
...
cmd_dispatch(struct panvk_cmd_buffer *cmdbuf, struct panvk_dispatch_info *info)
{
   ...
   #if PAN_ARCH < 9
   /* Bifrost path (v6/v7): uses COMPUTE_JOB sections INVOCATION, PARAMETERS, DRAW */
...
#else
   /* JM/Valhall v9 path: Compute Job = Header + Compute Payload.
    * The Compute Payload contains workgroup params + Shader Environment (SPD).
    * No INVOCATION, PARAMETERS, or DRAW sections exist for v9.
    * Shader Environment layout (v9.xml):
    *   d[0] = attribute_offset
    *   d[1] = fau_count
    *   d[2-7] = padding
    *   d[8-9] = resources
    *   d[10-11] = shader (SPD address)
    *   d[12-13] = thread_storage
    *   d[14-15] = FAU
    */
   struct pan_ptr job = panvk_cmd_alloc_desc(cmdbuf, COMPUTE_JOB);
   if (!job.gpu)
   ...
   util_dynarray_append(&batch->jobs, job.cpu);
    ...
    /* Pack Compute Payload using pan_section_pack for v9 */
    pan_section_pack(job.cpu, COMPUTE_JOB, PAYLOAD, cfg) {
       ...
       /* Shader Environment: use SPD instead of RSD */
       cfg.compute.attribute_offset = 0;
       cfg.compute.fau_count = cs->fau.total_count;
       cfg.compute.resources = cmdbuf->state.compute.cs.desc.res_table;
