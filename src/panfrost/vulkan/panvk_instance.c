...
{
   PFN_vkVoidFunction fn = panvk_GetInstanceProcAddr(instance, pName);
   if (fn == NULL && instance != NULL) {
      VK_FROM_HANDLE(panvk_instance, panvk_inst, instance);
      fn = vk_instance_get_proc_addr_unchecked(&panvk_inst->vk, pName);
   }
   return fn;
}
