                      ...
                      struct panvk_shader *shader)
{
   ...
   case MESA_SHADER_FRAGMENT:
      ...
      break;
   /* ADHOC-DXVK-FL: DXVK binds the geometry stage (possibly NULL) whenever
    * geometryShader is advertised.  panvk cannot execute geometry shaders on
    * Bifrost, so ignore the binding entirely instead of asserting.
    */
